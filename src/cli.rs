...
fn run_cli(cli: Cli) -> std::result::Result<(), CliRunError> {
    match cli.command {
        ...
        Some(CliCommand::Uninstall) => {
            ...
            eprintln!("petiglyph: {}uninstall is ambiguous.{}", colors.bold_red, colors.reset);
            eprintln!("Did you mean `{}` or `{}`?", "uninstall-font", "nuke-everything");
            eprintln!();
            eprintln!("  {}{}{}  - Removes only the font variants generated by your active project.", colors.bold, "uninstall-font", colors.reset);
            eprintln!("  {}{}{}   - Deletes all petiglyph traces on this machine (all fonts, all projects, all metadata). NUKED AND LOST.", colors.bold, "nuke-everything", colors.reset);
            std::process::exit(1);
        }
